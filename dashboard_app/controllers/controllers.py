...
# 'クローズ_本日.xlsm'ファイル
close_file = r'\\mjs.co.jp\datas\CSC共有フォルダ\第47期 東京CSC第二グループ\47期SV共有\ダッシュボードアイテム\files\todays_op.xlsm'

# 活動ファイル
activity_path = r"\\mjs.co.jp\datas\CSC共有フォルダ\第47期 東京CSC第二グループ\47期SV共有\ダッシュボードアイテム\files"
activity_file = r"todays_activity.xlsm"
...
def get_today_acw_att_cph_to_html():
    ...
    ratio_direct_other = df_direct_ratio.loc['その他', '直受け']
    ratio_direct_all = df_direct_ratio.loc['総計', '直受け']

    ratio_20_2g = df_ratio.loc['第2G', '20分以内']
    ratio_20_3g = df_ratio.loc['第3G', '20分以内']
    ratio_20_n = df_ratio.loc['長岡', '20分以内']
    ratio_20_other = df_ratio.loc['その他', '20分以内']
    ratio_20_all = df_ratio.loc['総計', '20分以内']
    ratio_40_2g = df_ratio.loc['第2G', '40分以内']
    ratio_40_3g = df_ratio.loc['第3G', '40分以内']
    ratio_40_n = df_ratio.loc['長岡', '40分以内']
    ratio_40_other = df_ratio.loc['その他', '40分以内']
    ratio_40_all = df_ratio.loc['総計', '40分以内']

    count_direct_2g = df_direct_count.loc['第2G', '直受け']
    count_direct_3g = df_direct_count.loc['第3G', '直受け']
    count_direct_n = df_direct_count.loc['長岡', '直受け']
    count_direct_other = df_direct_count.loc['その他', '直受け']
    count_direct_all = df_direct_count.loc['総計', '直受け']

    count_20_2g = df_count.loc['第2G', '20分以内']
    count_20_3g = df_count.loc['第3G', '20分以内']
    count_20_n = df_count.loc['長岡', '20分以内']
    count_20_other = df_count.loc['その他', '20分以内']
    count_20_all = df_count.loc['総計', '20分以内']
    count_40_2g = df_count.loc['第2G', '40分以内']
    count_40_3g = df_count.loc['第3G', '40分以内']
    count_40_n = df_count.loc['長岡', '40分以内']
    count_40_other = df_count.loc['その他', '40分以内']
    count_40_all = df_count.loc['総計', '40分以内']

    count_direct_all_2g = df_direct_count.loc['第2G', '電話対応数']
    count_direct_all_3g = df_direct_count.loc['第3G', '電話対応数']
    count_direct_all_n = df_direct_count.loc['長岡', '電話対応数']
    count_direct_all_other = df_direct_count.loc['その他', '電話対応数']
    count_direct_all_all = df_direct_count.loc['総計', '電話対応数']
    count_all_2g = df_count.loc['第2G', '指標集計対象']
    count_all_3g = df_count.loc['第3G', '指標集計対象']
    count_all_n = df_count.loc['長岡', '指標集計対象']
    count_all_other = df_count.loc['その他', '指標集計対象']
    count_all_all = df_count.loc['総計', '指標集計対象']
    ...
    # 保存先file path
    dashboard_path_2g = r'\\mjs.co.jp\datas\CSC共有フォルダ\第47期 東京CSC第二グループ\リアルタイムKPI'
    index_path_2g = r'\\mjs.co.jp\datas\CSC共有フォルダ\第47期 東京CSC第二グループ\リアルタイム個人別パフォーマンス'
    monitor_path_2g = r'\\mjs.co.jp\datas\CSC共有フォルダ\第47期 東京CSC第二グループ\リアルタイムKPI_モニター用'
    dashboard_2g = os.path.join(dashboard_path_2g, 'dashboard.html')
    index_2g = os.path.join(index_path_2g, 'index.html')
    monitor_2g = os.path.join(monitor_path_2g, 'monitor.html')
    ...
    with open(monitor_2g, 'w') as f_monitor:
        f_monitor.write(html_to_monitor)
    

     # パフォーマンス用テンプレートの読込み
    with open(r'C:\Users\eisuke_koresawa\Desktop\project\dashboard2\dashboard_app\templates\templates_performance.txt', 'r') as template_file:
